...
    if (
      !payload.name || !payload.address || !payload.farmLocation ||
      !payload.mobile || !payload.email || !password || !confirmPassword
    ) {
...
      router.replace('/(stakeholder)/tabs/stakeholder-home');
    } catch (e: any) {
      const msg = e?.response?.data?.error ?? e.message ?? 'Failed to register.';
      setError(msg);
    } finally {
      setBusy(false);
    }
...
        <TextInput style={styles.input} placeholder="Juan Dela Cruz" value={name} onChangeText={t => setName(cap(t))} />

        <View style={styles.row}>
          <View style={{ flex: 1, marginRight: 8 }}>
            <Text style={styles.label}>Address *</Text>
            <TextInput style={styles.input} placeholder="Brgy. Poblacion" value={address} onChangeText={t => setAddress(cap(t))} />
          </View>
          <View style={{ flex: 1, marginLeft: 8 }}>
            <Text style={styles.label}>Farm Location *</Text>
            <TextInput style={styles.input} placeholder="Valencia City" value={farmLocation} onChangeText={t => setFarmLocation(cap(t))} />
          </View>
        </View>
...
        <TouchableOpacity style={[styles.registerButton, busy && { opacity: 0.7 }]} onPress={handleRegister} disabled={busy}>
...
  logo: { width: 160, height: 160, alignSelf: 'center', marginTop: -40, marginBottom: -30 },
  tabContainer: { flexDirection: 'row', justifyContent: 'center', marginBottom: 15 },
  tabActive: { fontSize: 15, fontWeight: 'bold', color: '#2e7d32', marginLeft: 16, borderBottomWidth: 2, borderBottomColor: '#2e7d32', paddingBottom: 1 },
...
  prefixBox: { backgroundColor: '#f0f0f0', paddingVertical: 12, paddingHorizontal: 14, borderTopLeftRadius: 8, borderBottomLeftRadius: 8, borderWidth: 1, borderColor: '#ccc' },
  prefixText: { fontSize: 14, color: '#333' },
  mobileInput: { flex: 1, borderWidth: 1, borderColor: '#ccc', borderLeftWidth: 0, borderTopRightRadius: 8, borderBottomRightRadius: 8, padding: 12, fontSize: 14 },
  registerButton: { backgroundColor: '#2e7d32', paddingVertical: 14, borderRadius: 50, marginTop: 10 },
